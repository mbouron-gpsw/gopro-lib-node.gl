...
def _get_glsl_fields_definition(set, binding, layout, name, definition):
    data_tpl = dict(name=name, definition=definition, set=set, binding=binding)
    if layout == 'std430':
        tpl = 'layout(std430, set=%(set)d, binding=%(binding)d) readonly buffer %(name)s_block {\n%(definition)s\n} %(name)s;'
    elif layout == 'std140':
        tpl = 'layout(std140, set=%(set)d, binding=%(binding)d) uniform %(name)s_block {\n%(definition)s\n} %(name)s;'
    else:
        tpl = '%(definition)s'
    return tpl % data_tpl
...
def filter_definition(prefix, definition, code):
    contents = ''
...
def get_render(cfg, quad, fields, block_definition, color_definition, block_fields, color_fields, layout, debug_positions=False):
    ...
    useBlocks = False
    if layout in ['std140', 'std430']: useBlocks = True
    block_definition = filter_definition('fields.' if useBlocks else '', block_definition, ''.join(func_definitions))
    color_definition = filter_definition('colors.' if useBlocks else '', color_definition, ''.join(func_definitions))

    frag_data = dict(
        block_definition=_get_glsl_fields_definition(2, 0, layout, 'fields', block_definition),
        color_definition=_get_glsl_fields_definition(3, 0, layout, 'colors', color_definition),
        layout=layout,
...
def get_random_block_info(spec, seed=0, layout=LAYOUTS[0], color_tint=True):
    ...
    if layout == 'uniform':
        ...
        color_fields = ngl.Block(fields=dict(color_fields), layout=layout, label='colors_block')
        block_fields = ngl.Block(fields=dict(block_fields), layout=layout, label='fields_block')
