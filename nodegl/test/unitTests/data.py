...
def _get_data_function(field_id, layout):
    if layout != 'std430': layout = 'std140' #uniforms outside a block not supported in Vulkan
...
layout(std140, set = 1, binding = 0) uniform streamed {
    vec4 data[%(data_size)d];
};
...
def _get_data_streamed_buffer_vec4_scene(cfg, scale, show_dbg_points):
    ...
    streamed_block = ngl.Block(layout='std140', label='streamed_block', fields={'data':streamed_buffer})

    shader_version = '300 es' if cfg.backend == 'gles' else '450'
