...
static int mc_map_st_frame(struct ngl_node *node, struct sxplayer_frame *frame)
{
    struct texture_priv *s = node->priv_data;
    struct hwupload *hwupload = &s->hwupload;
    struct media_priv *media = s->data_src->priv_data;
...
static int mc_map_ir_frame(struct ngl_node *node, struct sxplayer_frame *frame)
{
    struct texture_priv *s = node->priv_data;
    struct hwupload *hwupload = &s->hwupload;
...
{
    struct ngl_ctx *ctx = node->ctx;
    struct android_api *api = &ctx->android_api;

    if (api->has_native_imagereader_api)
        return mc_map_ir_frame(node, frame);

    return mc_map_st_frame(node, frame);
}
