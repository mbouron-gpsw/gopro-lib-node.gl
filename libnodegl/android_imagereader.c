...
{
    struct android_api *api = &s->api;

    AHardwareBuffer *hardware_buffer;
    media_status_t status = api->AImage_getHardwareBuffer(s->image, &hardware_buffer);
    if (status != AMEDIA_OK)
        return NULL;
...
struct android_imagereader *ngli_android_imagereader_create(struct android_api *api, int width, int height, int format, int max_images)
{
    if (!api->has_native_imagereader_api)
        return NULL;
    ...
    media_status_t status = api->AImageReader_new(width, height, format, max_images, &s->reader);
    if (status != AMEDIA_OK) {
    ...
    status = api->AImageReader_setImageListener(s->reader, &listener);
    if (status != AMEDIA_OK) {
...
fail:
    ...
    api->AImageReader_delete(s->reader);
    ngli_freep(&s);
...
int ngli_android_imagereader_get_window(struct android_imagereader *s, void **window)
{
    ...
    media_status_t status = api->AImageReader_getWindow(s->reader, &native_window);
    if (status != AMEDIA_OK)
    ...
    jobject object = api->ANativeWindow_toSurface(env, native_window);
    *window = (*env)->NewGlobalRef(env, object);
...
int ngli_android_imagereader_acquire_next_image(struct android_imagereader *s, struct android_image **imagep)
{
    ...
    pthread_mutex_lock(&s->lock);
    for (;;) {
        status = api->AImageReader_acquireNextImage(s->reader, &android_image);
    ...
    if (!image) {
        api->AImage_delete(android_image);
