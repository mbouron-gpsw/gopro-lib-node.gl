 ...
 * under the License.
 */
#include "porting/vulkan/VKGraphics.h"
#include "porting/vulkan/VKGraphicsContext.h"
#include "porting/vulkan/VKBuffer.h"
#include "porting/vulkan/VKComputePipeline.h"
#include "porting/vulkan/VKGraphicsPipeline.h"
...
void VKGraphics::bindTexture(CommandBuffer* commandBuffer, Texture* texture, uint32_t set) {
    ...
    if (VKGraphicsPipeline* graphicsPipeline = dynamic_cast<VKGraphicsPipeline*>(currentPipeline)) {
        if (!(vkTexture->imageUsageFlags & VK_IMAGE_USAGE_SAMPLED_BIT)) {
            ERR("incorrect image usage flags: missing IMAGE_USAGE_SAMPLED_BIT");
        }
        ...
        if (!(vkTexture->imageUsageFlags & VK_IMAGE_USAGE_STORAGE_BIT)) {
            ERR("incorrect image usage flags: missing IMAGE_USAGE_STORAGE_BIT");
        }
